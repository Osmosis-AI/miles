...
required_paths=(
    ...
    "${MILES_ROOT}/scripts/models/llama3.2-3B-Instruct.py"
    "${DATA_PATH}"
    "${STUDENT_HF_PATH}"
...
mkdir "${RAY_TEMP_DIR}"
ray_temp_owned=1
# Model-arch args come from scripts/models/llama3.2-3B-Instruct.py via the
# helper that collapses them to one shell-readable line.
MODEL_ARGS_LINE="$(python3 "${MILES_ROOT}/miles/utils/external_utils/model_args_utils.py" "llama3.2-3B-Instruct")" || exit 1
