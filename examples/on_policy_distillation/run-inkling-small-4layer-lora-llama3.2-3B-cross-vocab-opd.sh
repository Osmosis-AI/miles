...
#   GPUs 0-3: colocated Inkling-Small 4-layer LoRA student (TP=4, EP=4) with
#             its SGLang rollout engines
#   GPU  4:   Llama-3.2-3B-Instruct SGLang teacher (TP=1)
#
# The student is the four-layer Inkling-Small LoRA smoke-test configuration
# (see 4layer_inkling_lora.md): the launcher mirrors the exact argument list
# `scripts/run_inkling.py train` produces for it, then swaps the math reward
# for the cross-tokenizer OPD hooks. The teacher tokenizer (Llama, 128k
# vocabulary) is disjoint from the Inkling tokenizer (201k vocabulary), so the
# teacher log-probabilities reach the student through the xToken aligner.
...
STUDENT_HF_PATH=${STUDENT_HF_PATH:-/data/Inkling-Small-chubby}
STUDENT_TORCH_DIST_PATH=${STUDENT_TORCH_DIST_PATH:-/data/Inkling-Small-4layer_torch_dist}
...
TEACHER_LOG_FILE=${TEACHER_LOG_FILE:-/tmp/sglang_llama3.2_3b_teacher_$$.log}
TRAIN_LOG_FILE=${TRAIN_LOG_FILE:-/tmp/miles_inkling_small_4layer_opd_train_$$.log}
...
required_paths=(
    ...
    "${MILES_ROOT}/scripts/models/inkling-small-4layer.sh"
    "${DATA_PATH}"
    "${STUDENT_HF_PATH}"
...
mkdir "${RAY_TEMP_DIR}"
ray_temp_owned=1
source "${MILES_ROOT}/scripts/models/inkling-small-4layer.sh"
