...
# xToken-aligned sampled-logprob OPD on one 8xH200 node, with an NVFP4
# pre-quantized student:
#   GPUs 0-3: colocated Inkling (975B config) 4-layer NVFP4 LoRA student
#             (TP=4, EP=4) with its SGLang rollout engines
#   GPU  4:   Llama-3.2-3B-Instruct SGLang teacher (TP=1)
#
# The student HF checkpoint is /data/Inkling-NVFP4-chubby: the first four
# layers of thinkingmachines/Inkling-NVFP4 (routed-expert weights stored as
# NVFP4; layers 0-1 dense and layer-2 experts excluded per its
# hf_quant_config.json). SGLang self-detects the Inkling NVFP4 quantization
# from hf_quant_config.json and, on Hopper (SM90), serves the quantized
# experts through the Marlin W4A16 weight-only fallback — no --quantization
# flag is needed. Training follows the canonical "NVFP4 rollout + BF16
# training" recipe (see scripts/run_deepseek_v3_5layer_nvfp4.py upstream):
# Megatron loads the matching BF16 torch_dist (/data/Inkling-4layer_torch_dist,
# converted from /data/Inkling-chubby), and since this is LoRA training the
# frozen base is never re-shipped to the rollout engines, so no NVFP4
# re-quantization happens during weight sync — only BF16 adapters flow.
#
# Otherwise identical to the Inkling-Small variant: the launcher mirrors the
# argument list `scripts/run_inkling.py train` produces for the 4-layer LoRA
# smoke test (4layer_inkling_lora.md), with the math reward swapped for the
# cross-tokenizer OPD hooks (teacher tokenizer Llama 128k vs Inkling 201k).
# W&B is intentionally not configured; this is a functional smoke run.
...
STUDENT_HF_PATH=${STUDENT_HF_PATH:-/data/Inkling-NVFP4-chubby}
STUDENT_TORCH_DIST_PATH=${STUDENT_TORCH_DIST_PATH:-/data/Inkling-4layer_torch_dist}
TEACHER_HF_PATH=${TEACHER_HF_PATH:-/data/Llama-3.2-3B-Instruct}
...
TEACHER_LOG_FILE=${TEACHER_LOG_FILE:-/tmp/sglang_llama3.2_3b_teacher_nvfp4_$$.log}
TRAIN_LOG_FILE=${TRAIN_LOG_FILE:-/tmp/miles_inkling_nvfp4_4layer_opd_train_$$.log}
...
required_paths=(
    ...
    "${MILES_ROOT}/scripts/models/inkling-975b-4layer.sh"
    "${DATA_PATH}"
    "${STUDENT_HF_PATH}"
...
mkdir "${RAY_TEMP_DIR}"
ray_temp_owned=1
source "${MILES_ROOT}/scripts/models/inkling-975b-4layer.sh"

# The blocks below mirror the argument list `scripts/run_inkling.py train`
# emits for the validated four-layer Inkling-Small LoRA smoke test
# (4layer_inkling_lora.md), with the math reward replaced by the
# cross-tokenizer OPD hooks.

CKPT_ARGS=(
    --hf-checkpoint "${STUDENT_HF_PATH}"
    --load "${STUDENT_TORCH_DIST_PATH}"
    --model-name inkling
...
# never appear in the shell trace.
set +x
ray_job_submitted=1
# The env_vars block reproduces the runtime environment scripts/run_inkling.py
# builds for the Inkling plugin path (SGLang server-engine logprobs, unified
# radix tree, NCCL knobs), merged with this demo's Ray/job settings.
