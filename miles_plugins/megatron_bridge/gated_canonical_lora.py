...
``kv_channels * num_attention_heads``, correct only for ungated attention.
Qwen3.5 uses gated attention, so ``q_proj`` outputs query + output gate
(``2 * num_attention_heads * head_dim``); this mirrors the bridge transform but
doubles ``q_out_features`` when ``attention_output_gate`` is set. The formula
reduces to the original for ungated models.
"""
...
def interleave_qkv_gated(self, query, key, value):
    ...
    The base implementation sizes q heads from ``num_attention_heads`` and cannot
    place the gated query (2x heads) into Megatron's per-group qkv layout.
    """
...
def patched_transform(self, m, name=None, prefix=None):
...
def install() -> None:
    cl.CanonicalLoRA.transform = patched_transform
