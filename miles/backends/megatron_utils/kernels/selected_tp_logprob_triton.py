...
def _partial_row_max_kernel(
    ...
    # Match the output dtype of the unfused linear projection before the
    # downstream float32 temperature/softmax computation.
    acc = acc.to(weight_ptr.dtype.element_ty).to(tl.float32)
    acc *= inv_temperature
...
def _partial_sum_selected_kernel(
    ...
    # The regular path materializes the projection in the model dtype, then
    # upcasts logits to float32 for log-probability and entropy calculations.
    acc = acc.to(weight_ptr.dtype.element_ty).to(tl.float32)
...
def _compute_forward_stats(
    ...
    def grid(meta):
        return (triton.cdiv(num_rows, meta["BLOCK_M"]), num_vocab_blocks)
...
class _FusedSelectedTPLogProbFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_log_prob: torch.Tensor, grad_entropy: torch.Tensor | None):
        ...
        for tile_start in range(0, local_vocab_size, _BACKWARD_VOCAB_TILE):
            ...
            # Autograd through `projection(...).float()` rounds the gradient
            # entering the model-dtype linear before its GEMMs. Reproduce that
            # boundary while accumulating the tiled GEMMs in float32.
            grad_logits_linear = grad_logits_tile.to(weight.dtype).float()
            if grad_hidden is not None:
                grad_hidden.add_(grad_logits_linear @ weight_tile.float())
            if grad_weight is not None:
                grad_weight_tile = grad_logits_linear.transpose(0, 1) @ hidden_states_fp32
                grad_weight[tile_start:tile_end] = grad_weight_tile.to(weight.dtype)
            if grad_bias is not None:
                grad_bias[tile_start:tile_end] = grad_logits_linear.sum(dim=0).to(bias.dtype)
