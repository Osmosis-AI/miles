...
def _find_output_layer(chunk: torch.nn.Module) -> tuple[torch.nn.Module | None, str | None]:
    """Find the actor LM head through both wrappers and nested bridge models."""
    try:
    ...
    candidates: dict[int, tuple[str, torch.nn.Module]] = {}
    for name, module in chunk.named_modules():
        if name and name.rsplit(".", 1)[-1] == "output_layer":
            candidates.setdefault(id(module), (name, module))

    if not candidates:
        return None, None
    if len(candidates) > 1:
        paths = ", ".join(sorted(name for name, _ in candidates.values()))
        raise RuntimeError(
            "Requested --use-chunked-tp-logprob-loss, but multiple nested actor "
            f"output_layer modules were found: {paths}"
        )
    path, output_layer = next(iter(candidates.values()))
    return output_layer, path
...
def validate_chunked_tp_logprob_config(args: Namespace) -> None:
    ...
    if args.train_backend != "megatron":
        raise ValueError("--use-chunked-tp-logprob-loss is only supported by the Megatron training backend.")
    if args.true_on_policy_mode:
        raise ValueError("--use-chunked-tp-logprob-loss does not support --true-on-policy-mode yet.")
    if args.enable_mtp_training:
        raise ValueError("--use-chunked-tp-logprob-loss does not support --enable-mtp-training yet.")
...
class ActorOutputProjection:
    ...
    def install_on(cls, model: torch.nn.Module | Sequence[torch.nn.Module]) -> ActorOutputProjection | None:
        ...
        for chunk in chunks:
            output_layer, path = _find_output_layer(chunk)
        ...
        if adapter is not None:
            logger.info(
                "chunked TP logprob bypass installed on %d output_layer(s): %s.",
                patched,
                ", ".join(patched_paths),
            )
        return adapter
    ...
    def fused_selected_logprob(
        ...
        *,
        ...
        if weight is None:
            raise RuntimeError("The actor output projection did not expose a weight tensor.")
...
def setup_chunked_tp_logprob(model: torch.nn.Module | Sequence[torch.nn.Module], args: Namespace, role: str) -> None:
    if not (
        getattr(args, "use_chunked_tp_logprob_loss", False)
        or getattr(args, "use_fused_tp_logprob_kernel", False)
    ):
        return
    ...
    if projection is None:
        ...
        if parallel_state.pp.rank == parallel_state.pp.size - 1:
            raise RuntimeError(
