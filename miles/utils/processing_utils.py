...
def extract_deferrable_media_refs(prompt) -> dict | None:
    ...
    None for them and the caller decodes eagerly, exactly as before. Returns None
    rather than {} when there is no media, so the caller can distinguish "defer
    this" from "nothing to defer".
    """
