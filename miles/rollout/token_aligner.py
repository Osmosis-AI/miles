...
"""xToken-style span alignment for miles cross-tokenizer OPD.

The DP kernel, canonicalization helpers, anchor optimization, and
post-processing supply the cross-tokenizer span alignment used by miles
on-policy distillation (OPD). This module does not implement projection-guided
P-KL or H-KL.
...
Public surface:
    ...
    - :class:`AlignmentBatch` — dense-padded per-batch alignment payload that
      records aligned spans and exact-match partitions.
    - :class:`TokenAligner` — owns the two tokenizers and exposes
      :meth:`align` for the collator.
"""
...
class AlignmentBatch:
    """Dense-padded per-batch span-alignment payload.

    Tensors are dense-padded to the batch maximum so consumers can shard on
    dim 0 without knowing about cross-tokenizer specifics.
    ...
    Attributes:
        ...
        student_exact_partition_mask: ``[B, T_s]`` bool. True at student
            tokens that sit on a 1-1 exact-match pair.
...
class TokenAligner:
    """Supply xToken-style span alignment for miles cross-tokenizer OPD.
    ...
    This class performs alignment only; it does not implement
    projection-guided P-KL or H-KL.
    ...
    def align(
        ...
        *,
        ...
        Returns:
            ...
            aligned spans and exact-match partitions.
        """
        ...
        for s_toks, t_toks in zip(student_token_lists, teacher_token_lists, strict=False):
    ...
    def _drop_padding(
        ...
        *,
        ...
        one side then has size 0 there and can be dropped by downstream
        filtering; a pair straddling the real/pad boundary shrinks to its real
        tokens.
        """
        ...
        *,
        ...
        for batch_i, pairs in enumerate(per_sample_pairs):
            ...
            for pair_i, pair in enumerate(pairs):
                ...
                # Exact partition: tokens on a 1-1 exact-match pair.
                if (
                    pair.is_correct
                    and pair.s_start != -1
                    and pair.t_start != -1
    ...
    def _align_single(
        ...
        for pair, m in zip(aligned, self._alignment_mask(aligned), strict=False):
            pair.is_correct = m
        return aligned
...
# These stay at module scope so callers can canonicalize tokens independently
# of ``TokenAligner``.
# =====================================================================
...
def canonical_token(token: str, *, enabled: bool = True) -> str:
    ...
    The ``enabled`` flag is a passthrough toggle: when ``False`` the input is
    returned unchanged, allowing callers to gate canonicalization without
...
def _merge_consecutive_bytes(
    ...
    for token, rng in zip(tokens, in_ranges, strict=False):
