...
async def generate(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    if deferred_refs and deferred_refs.get("image"):
        # The engine is colocated and sees the same filesystem, so hand it the
        # paths. Re-encoding the decoded PIL back to PNG costs ~84 ms per page
        # (~44 s per rollout step here) on the same asyncio thread that submits
        # requests, and inflates each request to megabytes of base64. The engine
        # applies the identical smart_resize to whatever it receives, so the
        # pixels reaching the model are the same either way.
