...
class TestSplitTrainDataRaw:
    def test_multi_lora_sort_preserves_witness_alignment(self) -> None:
        data = {
            "tokens": [[i] for i in range(6)],
            "response_lengths": [1] * 6,
            "loss_masks": [[1]] * 6,
            "adapter_slots": [2, 0, 1, 2, 0, 1],
            "seq_witness_ids": [torch.tensor([i]) for i in range(6)],
        }

        args = MagicMock()
        args.balance_data = False
        args.multi_lora_n_adapters = 4

        result = split_train_data_by_dp_raw(args, data, dp_size=2)

        assert [part["partition"] for part in result] == [[4, 2, 0], [1, 5, 3]]
        assert [part["adapter_slots"] for part in result] == [[0, 1, 2], [0, 1, 2]]
        assert [[x.item() for x in part["seq_witness_ids"]] for part in result] == [[4, 2, 0], [1, 5, 3]]
        assert [part["n_adapters"] for part in result] == [4, 4]
