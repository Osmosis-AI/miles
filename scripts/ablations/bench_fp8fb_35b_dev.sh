...
OUT_ROOT="${OUT_ROOT:-/weka/ablations/qwen3-5-35b-fp8fb-dev}"
USE_FP8="${USE_FP8:-0}"
if [ "${USE_FP8}" = "1" ]; then
...
[ "${FP8_FROZEN_BASE:-0}" = "1" ] && PRECISION_ARGS+=(--fp8-frozen-base-store)
[ "${FP8_FB_PER_LAYER:-0}" = "1" ] && PRECISION_ARGS+=(--fp8-frozen-base-per-layer-free)
...
for label in "${LABEL_PREFIX}baseline"; do
    echo "--- ${label} ---"
